...
;; The PROC language
;; Write a curried procedure to simulate the procedure take multiple
;; argument
...
;; curried function simulate take multiple argument
(define program1
  "let f = proc (x) proc (y) -(x, y)
   in ((f 3) 2)")

(define program2
  "let same? = proc (x) proc (y) zero?(-(x, y))
   in ((same? 3) 3)")
...
(check-expect (value-of program2) (bool-val #t))
(check-expect (value-of program3) (bool-val #f))
