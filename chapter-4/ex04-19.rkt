...
;; multiprocedure letrec so that each closure is built once
...
(define extend-env-recs
  (lambda (p-names  b-vars bodys saved-env)
    (if (null? p-names)
        saved-env
        (extend-env-recs
         (cdr p-names) (cdr b-vars) (cdr bodys)
         (extend-env-rec (car p-names) (car b-vars) (car bodys)
                         saved-env)))))
  ...
  (lambda (exp env)
    (cases expression exp
      ...
      [letrec-exp (proc-names bound-vars proc-bodys letrec-body)
                  (let [(nenv (extend-env-recs proc-names bound-vars 
                                               proc-bodys env))]
...
(define program7
  "letrec double (x) = if zero?(x) then 0 else -((double -(x, 1)), -2)
          sum (n) = if zero?(n) then 0 else -((sum -(n, 1)), -(0, n))
   in (sum (double 50))")
...
(check-expect (value-of program5) (num-val 12))
(check-expect (value-of program6) (num-val 55))
(check-expect (value-of program7) (num-val 5050))
